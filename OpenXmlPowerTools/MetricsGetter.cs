...
namespace OpenXmlPowerTools
{
    ...
    public class MetricsGetter
    {
        ...
        {
            WmlDocument noTrackedRevisions = new WmlDocument(wmlDoc);
            ...
            try
            {
                ...
                using (WordprocessingDocument document = streamDoc.GetWordprocessingDocument())
                {
                    ...
                    return GetWmlMetrics(wmlDoc.FileName, false, document, noTrackedDocument, settings);
                }
            }
            catch (OpenXmlPowerToolsException e)
            {
                if (e.ToString().Contains("Invalid Hyperlink"))
                {
                    ...
                    {
                        WmlDocument fixedWmlDoc = new WmlDocument(wmlDoc);
                    ...
                    noTrackedRevisions = new WmlDocument(wmlDoc);
                    using (OpenXmlMemoryStreamDocument noTrackedStreamDoc = new OpenXmlMemoryStreamDocument(noTrackedRevisions))
                    using (WordprocessingDocument noTrackedDocument = noTrackedStreamDoc.GetWordprocessingDocument())
                    using (OpenXmlMemoryStreamDocument streamDoc = new OpenXmlMemoryStreamDocument(wmlDoc))
                    using (WordprocessingDocument document = streamDoc.GetWordprocessingDocument())
                    {
                        if (RevisionAccepter.HasTrackedRevisions(noTrackedDocument))
                            RevisionAccepter.AcceptRevisions(noTrackedDocument);
                        return GetWmlMetrics(wmlDoc.FileName, true, document, noTrackedDocument, settings);
                    }
                }
            }
        ...
        private static bool ValidateWordprocessingDocument(WordprocessingDocument wDoc, List<XElement> metrics, List<string> notes, Dictionary<XName, int> metricCountDictionary)
        {
            ...
            if (!valid)
            {
                ...
                foreach (var err in errors.Take(1))
                {
                    if (err.Description.ToLower().Contains("smarttag"))
                    {
                        metrics.Add(new XElement(H.SmartTag, true));
                        valid = false;
                    }
                    else
                    {
                        if (err.Description.Length > 300)
                            notes.Add(MakeValidXml(err.Description.Substring(0, 300) + " ... elided ..."));
                        else
                            notes.Add(MakeValidXml(err.Description));
                        notes.Add("  in part " + MakeValidXml(err.Part.Uri.ToString()));
                        notes.Add("  at " + MakeValidXml(err.Path.XPath));
                    }
                }
            }
            int elementCount = 0;
            ...
            XDocument settingsXdoc = wDoc.MainDocumentPart.DocumentSettingsPart.GetXDocument();
        ...
        private static void NumberingFormatListAssembly(WordprocessingDocument wDoc, List<XElement> metrics)
        {
            ...
            if (numFmtList.Any())
            {
                ...
                metrics.Add(new XElement(H.NumberingFormatList, new XAttribute(H.Val, MakeValidXml(nfls))));
            }
        }
        ...
        private static void FontAndCharSetAnalysis(WordprocessingDocument wDoc, List<XElement> metrics, List<string> notes)
        {
            ...
            if (formattingMetrics.Languages.Any())
            {
                ...
                metrics.Add(new XElement(H.Languages, new XAttribute(H.Val, MakeValidXml(uls))));
            }
        }
        ...
        private static void AnalyzeRun(XElement run, List<XElement> attList, List<string> notes, FormattingMetrics formattingMetrics, string uri)
        {
            ...
            if (rPr == null)
            {
                ...
                notes.Add(MakeValidXml(string.Format("Error in part {0}: run without rPr at {1}", uri, run.GetXPath())));
        ...
        public static XElement GetXlsxMetrics(SmlDocument smlDoc, MetricsGetterSettings settings)
        {
            ...
            {
                using (SpreadsheetDocument spreadsheet = streamDoc.GetSpreadsheetDocument())
                {
                    return new XElement(H.Metrics,
                        new XAttribute(H.FileName, smlDoc.FileName),
                        new XAttribute(H.FileType, "SpreadsheetML"),
                        GetTableInfoForWorkbook(spreadsheet, settings));
                }
            }
        }
        ...
        public static XElement GetPptxMetrics(PmlDocument pmlDoc, MetricsGetterSettings settings)
        {
            ...
            {
                using (PresentationDocument presentation = streamDoc.GetPresentationDocument())
                {
                    return new XElement(H.Metrics,
